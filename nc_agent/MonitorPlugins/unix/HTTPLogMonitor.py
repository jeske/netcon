...
class HTTPLogMonitor (IncLogMonitor.IncLogMonitor):
    def collectData(self, config):
        logfile = config

        logline_re = re.compile('(\S+) \S+ \S+ \[[^\]]+\] "[^"]+" (\d+) (\d+|-)')
...
def makeMonitor(ncc):
    return HTTPLogMonitor(ncc)

def test():
    class DummyNCC:
        def newData(self,*a):
            print string.join(map(lambda x:str(x),a)," ")
    mon = HTTPLogMonitor(DummyNCC())
    mon.collectData('/httpd/logs/access_log')
